...
class Program
{
    ...
    static void Main()
    {
        ...
        //TEST CASE
        // 3. Test AddFirst
        Console.WriteLine("\n=== 2. Test AddFirst (Thêm 'Bình' vào đầu) ===");
        list.Addfirst(new Student("250011000", "Bình"));
        list.PrintForward();

        // 4. Test GetSize
        Console.WriteLine($"\n=== 3. Test GetSize ===\nSố lượng hiện tại: {list.GetSize()}");

        // 5. Test InsertAfterIndex (Chèn 'Cường' sau vị trí index 1 - sau 'Hải')
        Console.WriteLine("\n=== 4. Test InsertAfterIndex (Chèn 'Cường' sau 'Hải') ===");
        list.InsertAfterIndex(1, new Student("250011555", "Cường"));
        list.PrintForward();

        // 6. Test RemoveNode (Xóa 'Hoàng' - mã 250011460)
        Console.WriteLine("\n=== 5. Test RemoveNode (Xóa 'Hoàng') ===");
        list.RemoveNode("250011460");
        list.PrintForward();
        // Kiểm tra in ngược để xác nhận các mối liên kết Prev vẫn đúng
        list.PrintBackward();

        // 7. Test Reverse
        Console.WriteLine("\n=== 6. Test Reverse (Đảo ngược toàn bộ danh sách) ===");
        list.Reverse();
        list.PrintForward();
        
        // Kiểm tra in ngược sau khi đảo để xem Tail có được cập nhật không
        Console.WriteLine("Kiểm tra in ngược sau khi Reverse:");
        list.PrintBackward();
    }
}
...
//Author: Hoang Khuong Duy - 2500114656
{
    ...
    public class SingleLinkedList
    {
        ...
        // 8.1 Hàm đếm và trả về tổng số lượng node có trong list
        public int CountNodes()
        {
            int count = 0; // Biến đếm số lượng Node
            Node current = head; // Bắt đầu từ head
            while (current != null) // Duyệt qua list
            {
        ...
        // 8.3 Xóa phần tử ở đầu danh sách. 
        // (Gợi ý: chỉ cần cho Head = Head.Next;)
        // Kiểm tra kỹ trường hợp danh sách rỗng
        ...
        {
            if (head == null) // Nếu list rỗng => không có gì để xóa
            {
                return;
            }
            ...
            Node current = head; // Bắt đầu từ head
            while (current.Next != null) // Duyệt list
            {
                if (current.Next.Data == value) // Nếu tìm thấy Node tiếp theo có data = value
                {
        ...
        public void ReverseList()
        {
            ...
            while (current != null) // Duyệt qua list
            {
                ...
                prev = current; // Cập nhạt prev thành Node hiện tại
    ...
    class Program
    {
        ...
        {
            // Thiết lập để hiển thị và nhập tiếng Việt có dấu
